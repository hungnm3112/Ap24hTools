...
| **Scraping** | Puppeteer | Cào dữ liệu + Auto-update giá |
...
│   ├── scraping/                # Module cào dữ liệu (Puppeteer + CronJob)
│   │   ├── scraping.module.ts
│   │   ├── scraping.controller.ts # Endpoint kích hoạt cào thủ công, proxy, gợi ý AI
│   │   ├── scraping.service.ts  # Chứa Puppeteer logic (mở trình duyệt, inject JS) và gọi AI
...
│   │   └── price-update.service.ts # Logic dùng Puppeteer giả lập thao tác Admin
...
| POST | `/api/matching/apply` | Thực thi cập nhật giá lên AP24h (Puppeteer) | JWT |
...
sequenceDiagram
    ...
    Backend->>Puppeteer: Mở trình duyệt, tải trang
    Puppeteer->>Target: Truy cập URL
    Target-->>Puppeteer: HTML response
    Puppeteer->>Puppeteer: Inject JS (highlight + click handler)
    Puppeteer-->>Backend: HTML đã inject
...
1. **Proxy Service (Backend):** Puppeteer tải trang → scroll hết trang (lazy load) → inject đoạn JS custom:
...
#### 4.2. Scraping Module
- [ ] Puppeteer Service: proxy tải trang web
...
- [ ] Puppeteer auto-login AP24h admin (sử dụng user_data)
...
# AP24h Admin (cho Puppeteer auto-update)
...
- [ ] Approve giá → Puppeteer tự động cập nhật lên AP24h admin
